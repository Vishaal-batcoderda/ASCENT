...
class TechnicalAgent:
    ...
    def analyze(
        ...
        bollinger = technical_service.calculate_bollinger_bands(
            history,
            bollinger_window,
        )
        ...
        return {
            ...
            "bollinger": {
                "upper": round(float(bollinger["upper"].dropna().iloc[-1]), 2),
                "middle": round(float(bollinger["middle"].dropna().iloc[-1]), 2),
                "lower": round(float(bollinger["lower"].dropna().iloc[-1]), 2),
            },
        }
